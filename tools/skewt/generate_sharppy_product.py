...
def main():
    ...
    ap.add_argument("--lat", type=float, default=-25.43)
    ap.add_argument("--lon", type=float, default=-49.27)
    ap.add_argument("--label", default="Curitiba, PR")
    ...
    for fh in hours:
        ...
        atomic_retrieve(client, pl, **common, levtype="pl", levelist="/".join(map(str, LEVELS)), param="130.128/131.128/132.128/157.128/156.128/135.128")
        ...
        fields = read_fields(pl, args.lat, args.lon); sflds = read_fields(sf, args.lat, args.lon); sflds.update(read_fields(oro, args.lat, args.lon))
    ...
    meta = {"model":"ECMWF IFS", "resolution":"0.25°", "lat":args.lat, "lon":args.lon, "location":args.label,
            "run":run_dt.strftime("%Y-%m-%d %HZ"), "forecast_hours":hours, "png_pattern":"f{fh:03d}.png",
            "gif":"animation.gif", "renderer":"SHARPpy viz.plotSkewT + SHARPpy viz.plotHodo", "browser_rendering":False}
