...
# All 27 Brazilian state capitals. Coordinates are the target point; Open-Meteo
# snaps the request to the selected ECMWF IFS 0.25-degree grid cell.
CAPITALS = [
    ("Rio Branco", "rio-branco", -9.9754, -67.8249),
    ("Maceió", "maceio", -9.6658, -35.7353),
    ("Macapá", "macapa", 0.0349, -51.0694),
    ("Manaus", "manaus", -3.1190, -60.0217),
    ("Salvador", "salvador", -12.9777, -38.5016),
    ("Fortaleza", "fortaleza", -3.7319, -38.5267),
    ("Brasília", "brasilia", -15.7939, -47.8828),
    ("Vitória", "vitoria", -20.3155, -40.3128),
    ("Goiânia", "goiania", -16.6869, -49.2648),
    ("São Luís", "sao-luis", -2.5307, -44.3068),
    ("Cuiabá", "cuiaba", -15.6014, -56.0979),
    ("Campo Grande", "campo-grande", -20.4697, -54.6201),
    ("Belo Horizonte", "belo-horizonte", -19.9167, -43.9345),
    ("Belém", "belem", -1.4558, -48.5039),
    ("João Pessoa", "joao-pessoa", -7.1195, -34.8450),
    ("Curitiba", "curitiba", -25.4284, -49.2733),
    ("Recife", "recife", -8.0476, -34.8770),
    ("Teresina", "teresina", -5.0892, -42.8019),
    ("Rio de Janeiro", "rio-de-janeiro", -22.9068, -43.1729),
    ("Natal", "natal", -5.7945, -35.2110),
    ("Porto Alegre", "porto-alegre", -30.0346, -51.2177),
    ("Porto Velho", "porto-velho", -8.7608, -63.8999),
    ("Boa Vista", "boa-vista", 2.8235, -60.6758),
    ("Florianópolis", "florianopolis", -27.5949, -48.5482),
    ("São Paulo", "sao-paulo", -23.5505, -46.6333),
    ("Aracaju", "aracaju", -10.9472, -37.0731),
...
def profile_at(obj: dict, index: int, lat: float, lon: float, location: str, valid: dt.datetime):
    ...
    if sp is not None and t2 is not None and td2 is not None:
        ...
        tmp.append(t2)
        dwpt.append(td2)
        uu.append(su if su is not None else 0.0)
        vv.append(sv if sv is not None else 0.0)
        omg.append(np.nan)
    ...
    for level in LEVELS:
        ...
        if not tdarr:
            rharr = as_series(hourly, hourly_name("relative_humidity", level))
        else:
            rharr = []
        ...
        if not tarr or not wsarr or not wdarr or not gharr:
            continue
        t = finite(tarr[index])
        td = finite(tdarr[index]) if tdarr else None
        if td is None and rharr:
            ...
            if t is not None and rh is not None:
                ...
                td = b * gamma / (a - gamma)
        ws = finite(wsarr[index])
        wd = finite(wdarr[index])
        gh = finite(gharr[index])
        if t is None or td is None or ws is None or wd is None or gh is None:
            continue
        u, v = to_uv(ws, wd)
        pres.append(p)
        hght.append(gh)
        tmp.append(t)
        dwpt.append(td)
        uu.append(u if u is not None else 0.0)
        vv.append(v if v is not None else 0.0)
...
def main():
    ...
    for level in LEVELS:
        variables += [
            hourly_name("temperature", level),
            hourly_name("dew_point", level),
            hourly_name("relative_humidity", level),
            hourly_name("wind_speed", level),
            hourly_name("wind_direction", level),
            hourly_name("geopotential_height", level),
            hourly_name("vertical_velocity", level),
        ]
    ...
    if isinstance(payload, dict):
        ...
        raise RuntimeError("Resposta Open-Meteo inesperada")
    if len(locations) != len(CAPITALS):
        raise RuntimeError(f"Open-Meteo retornou {len(locations)} locais; esperado {len(CAPITALS)}")

    renderer = load_renderer()
    out_root = Path(args.out)
    out_root.mkdir(parents=True, exist_ok=True)
    manifest = {
        "schema": "sideral-capitals-skewt-sharppy-v1",
        "provider": "Open-Meteo Single Runs",
        "model": "ECMWF IFS 0.25°",
        "renderer": "SHARPpy",
        "run_utc": run.strftime("%Y-%m-%d %H:%MZ"),
        "cycle": args.cycle,
    ...
    for capital, obj in zip(CAPITALS, locations):
        ...
        if not times:
            raise RuntimeError(f"Sem eixo temporal para {capital[0]}")
        ...
        for fh in HOURS:
            ...
            if valid not in parsed_times:
                raise RuntimeError(f"{capital[0]}: F{fh:03d} não está disponível no run {run:%Y-%m-%d %HZ}")
            idx = parsed_times.index(valid)
            sounding = profile_at(obj, idx, capital[2], capital[3], capital[0], valid)
            output = city_dir / f"{capital[1]}-f{fh:03d}.png"
            print(f"[SHARPpy] {capital[0]} F{fh:03d}", flush=True)
            render_one(renderer, sounding, capital, fh, output)
            gc.collect()
