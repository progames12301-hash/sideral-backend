...
[[ "$ICON_SOURCE_XSIZE" =~ ^[0-9]+$ && "$ICON_SOURCE_YSIZE" =~ ^[0-9]+$ ]] || { echo "Tamanho da grade fonte ICON invalido" >&2; exit 2; }
for VALUE in "$ICON_SOURCE_XFIRST" "$ICON_SOURCE_YFIRST" "$ICON_SOURCE_XINC" "$ICON_SOURCE_YINC"; do [[ "$VALUE" =~ ^-?[0-9]+([.][0-9]+)?$ ]] || { echo "Grade fonte ICON invalida: $VALUE" >&2; exit 2; }; done

if [[ -n "${FORCE_RUN_DATE:-}" && -n "${FORCE_RUN_CYCLE:-}" ]]; then
  RUN_DATE="$FORCE_RUN_DATE"; RUN_CYCLE="$(printf '%02d' "$((10#$FORCE_RUN_CYCLE))")"
else
...
PY
  ...
  for C in "${CANDIDATES[@]}"; do
    ...
    if curl -fsSL --range 0-0 --connect-timeout 15 --max-time 45 -o /dev/null "$URL"; then RUN_DATE="$DATE"; RUN_CYCLE="$CYCLE"; break; fi
  done
...
export SOURCE_MODEL=icon RUN_DATE RUN_CYCLE WRF_RUN_HOURS WRF_START_HOUR WRF_END_HOUR
export SOURCE_DIR="$REG_DIR" SOURCE_VTABLE="$ROOT/wrf/Vtable.ICONp"
