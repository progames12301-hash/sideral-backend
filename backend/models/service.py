...
from .processing import combine_fields, common_grid, load_field, probability_exceedance, regrid_to_common_grid
...
class ModelService:
    def __init__(self, data_dir: Path = MODEL_DATA_DIR, cache_dir: Path = MODEL_CACHE_DIR) -> None:
        ensure_backend_directories()
        self.storage=ModelStorage(data_dir)
    ...
    def _single_field(self, model: str, run: str, product: str, region: str, forecast_hour: int) -> tuple[Field,Path]:
        path=self.storage.frame_path(model,run,product,region,forecast_hour,"npz")
        if not path:raise NotFound("Campo numérico ainda não disponível.")
        definition=PRODUCT_BY_ID[product]
        try:
            field=load_field(path,model=model,product=product,run=run,forecast_hour=forecast_hour)
    ...
    def frame(self, model: str, run: str, product: str, region: str, forecast_hour: int, metadata_only: bool = False) -> tuple[dict[str,Any] | bytes,str,dict[str,str]]:
        ...
        if metadata_only:
            ...
            if not png and not npz:raise NotFound("Dado indisponível para esta seleção.")
            return {"model":model,"run":run,"product":product,"region":region,"forecast_hour":forecast_hour,"valid_time":self._valid_time(run,forecast_hour),"png_available":bool(png),"field_available":bool(npz)},"application/json",{}
        if png:
            body=png.read_bytes()
            return body,"image/png",{"X-Sideral-Source":"preprocessed","ETag":hashlib.sha256(body).hexdigest()}
        field,path=self._single_field(model,run,product,region,forecast_hour)
        stat=path.stat();key=self.cache.digest(["single",model,run,product,region,str(forecast_hour),str(stat.st_mtime_ns),str(stat.st_size)])
    ...
    def _multimodel_fields(self,run: str,product: str,region: str,forecast_hour: int) -> tuple[list[Field],list[str]]:
        definition=PRODUCT_BY_ID[product]
        ...
        accepted: list[Field]=[];models_used: list[str]=[]
        for model in self._operational_models():
            ...
            try:
                ...
                accepted.append(field);models_used.append(model)
    ...
    def multimodel_frame(self,run: str,product: str,region: str,forecast_hour: int,statistic: str) -> tuple[bytes,str,dict[str,str]]:
        ...
        fields,models_used=self._multimodel_fields(run,product,region,forecast_hour)
        ...
        for model in models_used:
            path=self.storage.frame_path(model,run,product,region,forecast_hour,"npz");stat=path.stat() if path else None
            fingerprints.append(f"{model}:{stat.st_mtime_ns}:{stat.st_size}" if stat else model)
    ...
    def probability(self,run: str,variable: str,region: str,forecast_hour: int,threshold: float,period: int | None = None) -> tuple[bytes,str,dict[str,str]]:
        ...
        fields,models_used=self._multimodel_fields(run,variable,region,forecast_hour)
        try:result,_=probability_exceedance(fields,float(threshold),MIN_MULTIMODEL_MEMBERS)
        except ValueError as exc:raise Conflict(str(exc)) from exc
        period_label=f"/{period}h" if period else ""
        fingerprints=[]
        for model in models_used:
            path=self.storage.frame_path(model,run,variable,region,forecast_hour,"npz");stat=path.stat() if path else None
            fingerprints.append(f"{model}:{stat.st_mtime_ns}:{stat.st_size}" if stat else model)
